...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _searchFromAddress() async {
    if (alamatCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Isi alamat dulu!")));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Lokasi tidak ditemukan. Coba tambahkan nama kota (contoh: Arjosari Pacitan)"),
  ...
  Future<void> _kirimData() async {
    if (tglP == null || tglB == null || idPelCtrl.text.isEmpty || selectedTeknisi == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lengkapi data penugasan!")));
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Penugasan PESTA Berhasil Disimpan!")));
        Navigator.pop(context);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text("DATA PERMINTAAN PLN", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
          const SizedBox(height: 15),
          TextField(
            controller: idPelCtrl, 
            decoration: const InputDecoration(labelText: "Nomor Agenda / Register", border: OutlineInputBorder()),
          ),
          const SizedBox(height: 15),
          TextField(
            controller: namaCtrl, 
            decoration: const InputDecoration(labelText: "Nama Pemohon (Sesuai PDF)", border: OutlineInputBorder()),
          ),
          const SizedBox(height: 15),
          ...
          TextField(
            ...
            onSubmitted: (_) => _searchFromAddress(), // Cari juga saat tekan 'Enter' di keyboard
          ),
          const SizedBox(height: 15),
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: "Daya VA", border: OutlineInputBorder(), suffixText: "VA"),
          ),
          const SizedBox(height: 25),
          
          const Text("PENJADWALAN", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
          ...
          isLoadingTeknisi 
            ...
            : DropdownButtonFormField<String>(
                ...
                items: availableTeknisi.map((u) => DropdownMenuItem(value: u.username, child: Text(u.nama))).toList(),
                onChanged: (val) => setState(() => selectedTeknisi = val),
              ),
          
          const SizedBox(height: 30),
          const Text("TITIK LOKASI PADA PETA", style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          SizedBox(
            height: 200,
            child: ClipRRect(
              ...
              child: FlutterMap(
                ...
                children: [
                  ...
                  MarkerLayer(markers: [
                    Marker(point: _selectedLocation, width: 40, height: 40, child: const Icon(Icons.location_on, color: Colors.red, size: 40)),
                  ]),
                ],
              ),
            ),
          ...
          ElevatedButton(
            ...
            child: const Text("SIMPAN & TUGASKAN", style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 20),
        ],
      ),
