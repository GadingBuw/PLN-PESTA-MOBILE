...
class AdminTechHistoryDetail extends StatelessWidget {
  ...
  // Fungsi Pengganti API get_history PHP
  Future<List<dynamic>> _fetchTechActiveTasks() async {
    try {
      // Ambil data dari Supabase: Filter berdasarkan teknisi & status BUKAN 'Selesai'
      final response = await supabase
          .from('pesta_tasks')
          .select()
          .eq('teknisi', username)
          .neq('status', 'Selesai'); // .neq berarti Not Equal (Tidak Sama Dengan)

      return response as List<dynamic>;
    } catch (e) {
      debugPrint("Error Supabase Detail: $e");
      return [];
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text("Tugas Aktif", style: TextStyle(fontSize: 10, color: Colors.white70)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text("Tidak ada tugas aktif saat ini.", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text("AGENDA: ${task['id_pelanggan']}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: Colors.grey, letterSpacing: 0.5)),
              Container(
          ...
          const SizedBox(height: 16),
          Text(task['nama_pelanggan'] ?? "-", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black87)),
          const SizedBox(height: 6),
          Row(
            children: [
              Icon(Icons.location_on_rounded, size: 14, color: primaryBlue),
              const SizedBox(width: 6),
              Expanded(child: Text(task['alamat'] ?? "-", style: const TextStyle(fontSize: 12, color: Colors.black54))),
            ],
          ),
          const Padding(
